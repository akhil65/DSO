...
**Garak is a red-teaming tool, not production middleware.** In a real org, the AppSec team or red team points Garak at the actual LLM endpoint — Azure OpenAI, the Anthropic API, an internal fine-tuned model hosted on your own inference cluster. You configure it with the right model type for that target:

```bash
# Against OpenAI / Azure OpenAI
garak --model_type openai --model_name gpt-4o --probes promptinject,dan

# Against any internal REST API (Claude API, Bedrock proxy, self-hosted model)
garak --model_type rest --model_name "https://your-internal-llm-api/v1/chat" \
      --probes promptinject,dan,encoding
```

The output tells you which attack families succeed against that specific model and configuration — not a generic model, yours. That feeds into a pre-deployment security review: "is this model safe to expose to users?" Results go to AppSec and engineering leads. If translation framing reliably extracts system prompt contents against your model in Garak, you know you need input scanning before go-live.

**LLM Guard is production middleware, not a developer tool.** It does not live on a developer's laptop — it gets containerised and deployed as middleware sitting between your users and the LLM API. Every user request passes through the input scanners before reaching the model; every model response passes through the output scanners before returning to the user. It runs 24/7, logs to your SIEM, and blocks at runtime.

**Dev → Staging → Production:** In development, engineers building LLM features should run basic prompt injection tests manually — the curl commands in Exercises 6.1 through 6.6 are a reasonable starting checklist for any LLM endpoint before it leaves the developer's laptop. In staging, Garak runs against the actual model endpoint (pointed at the staging API, not a local Ollama instance) to assess the model's resistance to known attack families. LLM Guard is deployed in staging in detection mode, and its logs are reviewed before production launch. In production, LLM Guard runs in blocking mode, logs feed into the SIEM, and AppSec monitors for novel attack patterns. The red team runs targeted prompt injection assessments quarterly, using current attack techniques that Garak's probe library may not yet cover.

**The end-to-end workflow:** Security team runs Garak against the target model in staging and finds that translation framing attacks succeed. That finding goes into a risk report. Engineering deploys LLM Guard with the PromptInjection scanner in front of the model. Security runs Garak again — the same probes now score 0.0 (all blocked). LLM Guard logs feed into the SIEM so the SOC can monitor for attack patterns in production. This is exactly what the lab demonstrates: Exercise 6.7 shows translation framing bypassing the model; Exercise 6.9 shows LLM Guard catching those same payloads at score 1.0.
...
**Lab vs real world:** In this lab, all of this is collapsed onto one machine — local model, local scanner, local red team. In practice these are three different teams touching three different parts of the pipeline. The AppSec team never touches the production model directly; the ML platform team is not writing Garak probes; the product engineers are not running the SIEM. The lab compresses that to make the relationships visible. The separation matters in a real org because each team needs different access, different tooling, and different reporting lines.
